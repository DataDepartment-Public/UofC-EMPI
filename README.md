...
Standardizes every field according to a fixed set of rules and produces a versioned cleaned CSV alongside derived columns (`last_4_SSN`, `ZipCD_clean_base`/`_ext`, `full_name_tokens`, `full_name_compact`, `Phones_set`, `Address_normalized`, and a record-level `valid_record` boolean).
...
- **Output location:** `data/processed/<stem>_cleaned_v<N>.csv` — version `N` is auto-incremented past the highest existing version for that input stem.
...
When re-reading a processed CSV elsewhere in code, use `src.data.clean.load_cleaned_csv(path)` so that leading zeros on `PATID`, `SSN`, `ZipCD`, and `last_4_SSN` are preserved.
...
- **Output location:** `src/features/outputs/blocking/candidate_pairs_v<version>_<timestamp>.parquet`. Schema: `PATID_A | PATID_B | source_blocks | n_blocks`.

### Run

```bash
# Defaults: --input data/processed/MDM_Population_cleaned_v1.csv,
#           --output src/features/outputs/blocking/, --version v1
python src/features/run_blocking.py

# Override any of the defaults
python src/features/run_blocking.py \
    --input  data/processed/MDM_Population_cleaned_v1.csv \
    --output src/features/outputs/blocking \
    --version v1
```
