...
## What's next: FS_Enhanced_3

Three working hypotheses surfaced during the E2-6 head-to-head review are captured here as scoping inputs for the next FS experiment.

**1. Anti-evidence factor problem (m/u inversion on three levels).** Supervised m-training from positives-only labels has a structural blind spot: levels that fire exclusively on negatives (`Household_discount`, `Sex_positive[M↔F]`, `SSN[9-digit mismatch]`) cannot be learned correctly from `estimate_m_from_pairwise_labels` — m falls to the Bayesian floor, and if u is also low, the weight inverts from negative to positive. The §27 m/u inspection (summarised in "Known limitations" item 2 above) is the primary evidence. Two remedies are worth evaluating for FS_Enhanced_3: (a) explicit negative labels for these levels in the synthetic training set, so `SupervisedTraining` has counter-examples to calibrate m, or (b) removal of the level from the FS comparison registry entirely and reliance on upstream `classify_non_matches` contradiction-filtering to handle those cases — consistent with the team's established "filter upstream, don't veto inside FS" design philosophy.

**2. Silver-label volume and training eligibility.** The `data/silver_labels/` held-out set contains n=204,805 entries — this matches the FULL candidate-pair count for run `real_20260620` (44,786 matches + 33,116 review + 126,903 rejects per `docs/Rejection-Rules-Analysis.md`), which strongly suggests the silver-label export captured the entire blocking output rather than only the Stage-3 confirmed matches. This requires verification before silver labels are used as ground-truth positives in FS_Enhanced_3 m-training. If the silver set is confirmed to be all-positive (i.e. the confirmed-matches subset only), the split-training procedure in `models/common/fs_base.py::SupervisedTraining` can incorporate real-PATID labels alongside the synthetic set, providing real-cohort m calibration that the synthetic generator cannot. Cross-reference `docs/Rejection-Rules-Analysis.md` for the full candidate-pair population breakdown.

**3. Upstream filter coverage as a training-pool contract.** `classify_non_matches` (Phase E2-5b, `src/models/deterministic_rules.py`) routes pairs with ≥ `DEFAULT_REJECT_MIN_CONTRADICTIONS` strong-identifier disagreements into `data/rejects/` before Stage 4 ever scores them. FS_Enhanced_2's calibration gap pre-merge was directly caused by training on a pool that mixed high-anti-evidence pairs (now in rejects) with genuinely ambiguous review-tier pairs. FS_Enhanced_3 must be trained and threshold-tuned exclusively against the post-`classify_non_matches` review pool — not the raw blocking output — to avoid re-introducing the same calibration gap. The corroboration-gate design scoped as Phase E2-5-fix5 (and deliberately dropped in E2-5b) remains available as a follow-up if post-merge real-cohort measurement on the review pool reveals residual auto_merge precision below threshold.
