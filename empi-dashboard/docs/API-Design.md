...
> schema + per-table status: `empi-service/docs/Data-Contract.md` Stage 6.
>
> This file is kept identical to `empi-service/docs/API-Design.md` (same
> content, cross-referenced from both packages) — update both together.
...
tables `record_attrs`/`record_raw`): `empi-service/docs/Data-Contract.md`
Stage 6.
...
`empi-service/docs/Explanations-Guide.md` — front-end readers only need
its §2.
