...
>
> This file is kept identical to `empi-service/docs/Application-Architecture.md`
> (same content, cross-referenced from both packages) — update both together.
...
| **Pipeline** (existing) | Python · pandas · pandera | `clean → block → rules → FS(audit) → gate → ML → cluster` (see `empi-service/docs/Data-Contract.md`) |
...
See `empi-service/docs/ML-Matcher-Integration-Guide.md` §1 and
`empi-service/docs/Nonmatch-Gate-Guide.md` for why the gate exists as a
separate stage rather than folding into the FS matcher or ML matcher.
...
`empi-service/docs/Data-Contract.md` Stage 6.
...
lookups — see `empi-service/docs/Data-Contract.md` Stage 6 for the full picture.)
...
6. **Operationalization** — incremental single/few-record scoring, a fully
   ...
   [API-Design.md](API-Design.md) and `empi-service/docs/Data-Contract.md`
   Stage 6.
