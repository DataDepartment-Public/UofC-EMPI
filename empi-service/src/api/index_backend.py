"""`IndexBackend` — the storage interface both `src/api/incremental.py`
(single/few-record scoring) and `src/api/publish.py` (full-batch publish)
read and write through, with two implementations:

* `SqlIndexBackend` — a thin, engine-agnostic adapter over a DB-API-style
  connection and a *store module* exposing `store.py`'s function surface
  (`lookup_block_candidates`, `upsert_entity`, ...). Defaults to
  `src.api.store` (SQLite today), but the connection is only ever typed as
...
* `ParquetIndexBackend` (`src/api/parquet_backend.py`) — the same operations
  against local Parquet files, read into memory and written back on commit.
  Used by the local-mode CLIs (`src/api/local_score.py`,
  `src/api/publish_local.py`), and by the API too when
...
performance reason `store.py`'s own bulk functions exist (see that module's
PERFORMANCE note).
"""
...
class SqlIndexBackend:
    ...
    current SQLite-flavored `src.api.store`; a future Postgres (or other
...
def build_index_backend(settings: Any) -> IndexBackend:
    ...
    (`"sqlite"` -> `SqlIndexBackend` over `store.py` / `settings.db_path`,
    ...
    store.init_db(conn)
    return SqlIndexBackend(conn, store_module=store)
