...
files (`ParquetIndexBackend`, `src/api/parquet_backend.py`), for local
dev/notebooks/CI use.

USAGE:
    python -m src.api.local_score --input record.json
    python -m src.api.local_score --input batch.json --data-dir data/local_index
...
from src.api import incremental
