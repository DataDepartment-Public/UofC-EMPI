...
`src/api/index_backend.py`) backed by plain Parquet files instead of
...
`src/api/local_score.py` and `src/api/deps.get_backend`'s process-local lock),
...
#: incremental scoring does). Matches `store._REVIEW_CANDIDATE_INSERT_SQL`'s
...
#: `run_id` deliberately excluded, matching `store.get_entity`'s SQL SELECT
...
class ParquetIndexBackend:
    ...
    def locked_patids(self) -> set[str]:
        ...
        `audit_log.patids`) — mirrors `store.locked_patids`. A later
    ...
    def max_mid_sequence(self) -> int:
        ...
        entity (see `store.max_mid_sequence`'s equivalent SQL role)."""
        best = 0
    ...
    # ── Bulk operations (batch publish — src/api/publish.py) ────────────────
    ...
    # reasoning as store.py's own bulk functions (see that module's
    # PERFORMANCE note).
    ...
    def replace_review_candidates_for_run(self, run_id: str, rows: list[tuple]) -> None:
        """Full per-run replace, mirroring `store.replace_review_candidates_for_run`
    ...
    def list_entities(
        ...
        *,
        ...
        """Pandas equivalent of `store.list_entities` — each filter narrows
    ...
    def dashboard_summary(self) -> dict:
        """Pandas equivalent of `store.dashboard_summary`."""
        entities = self._tables["entity"]
        members = self._tables["entity_member"]
        audit_action_counts = {
            str(k): int(v)
    ...
    def review_candidates_for_patid(self, patid: str) -> list[dict]:
        """Pandas equivalent of `store.review_candidates_for_patid` — joins
