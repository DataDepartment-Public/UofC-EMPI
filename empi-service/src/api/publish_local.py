...
`src/api/parquet_backend.py`), for a fully self-contained local/CI batch
workflow with zero SQLite dependency:
    ...
    python -m src.api.publish_local --run-id <id>                # this module —
                                                                  # materializes that
                                                                  # run into data/local_index/

USAGE:
    python -m src.api.publish_local --run-id 20260714T120000Z
    python -m src.api.publish_local --run-id 20260714T120000Z --data-dir data/local_index
...
is `src/api/local_score.py`; both are `ParquetIndexBackend`-backed and share
