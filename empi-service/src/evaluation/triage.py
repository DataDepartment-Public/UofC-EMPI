...
What this measures is the **shipped** decision. While `ml_feeds_clustering` is
off, the ML matcher's `auto_merge` tier changes no output, so those pairs land
in `human_review` here — correctly, because that is where they end up. The
