"""Object-oriented base for the production Fellegi-Sunter matcher.

Lifted (trimmed) into the production `fs_matcher` package from the former
`models/common/fs_base.py`. Three concepts live here:

* `ComparisonSpec` / `ComparisonRegistry` — a declarative, ordered,
  mutable-by-copy collection of Splink comparisons. `FSMatcher` assembles its
  registry once at import and downstream code can build new registries via
  `with_added` / `with_removed` / `with_replaced` without mutating the original.

* `TrainingStrategy` (ABC) + concrete `SupervisedTraining` — the m/u estimation
  procedure. `SupervisedTraining` trains m directly from a labeled-pairs frame
  (the silver labels) and estimates u by random sampling on the real cohort. The
  EM strategy from the research base is intentionally omitted — production trains
  supervised only.

* `FSModel` (ABC) — owns Splink-linker boilerplate, the n_blocks log-odds bump,
  threshold classification, and the two output projections (the ProbabilisticMatches
  audit frame + the eval-schema projection). Subclasses configure `model_name`,
  `registry`, `training`, and `classification_config`, and implement
  `prepare_model_input` + `build_settings`.
...
class ComparisonSpec:
    ...
    `builder` is a zero-arg callable returning the comparison's dict form (the
    shape a Splink Linker accepts inside `settings["comparisons"]`). It is
...
class SupervisedTraining(TrainingStrategy):
    ...
    handle this, pass `labels_records_df` — a records frame whose PATIDs the
    labels DO reference. The strategy then trains m on an auxiliary linker bound
    to that frame and copies the trained m into the live linker, estimating u on
    the live (real-cohort) linker.
    """
    ...
    def __init__(
        ...
        seed: int = 42,
        labels_table_name: str = "silver_labels",
    ...
    def train(
        ...
        if self.labels_records_df is None:
            # Single-linker path. Labels' PATIDs must be in df_clean.
...
# ═══════════════════════════════════════════════════════════════════════════════
class FSModel(ABC):
    """Abstract base for one FS matcher.

    Subclass responsibilities (class or instance attributes):
    ...
    def to_probabilistic_matches(self, df_classified: pd.DataFrame) -> pd.DataFrame:
        """Project to the ProbabilisticMatches contract (the FS audit frame).

        Default projection omits `veto_reason`; `veto_reason` is optional in the
        pandera contract, so a model that produces a veto column can subclass and
        override this method to include it.
        """
        required = {
    ...
    def run(
        ...
        second element of a `(result, linker)` tuple.
        """
...
def _copy_m_probabilities(src_linker: Any, dst_linker: Any) -> None:
    ...
    Used by the split-training path in `SupervisedTraining`. The two linkers must
    share the same comparison structure (same registry). Levels with
    `m_probability is None` on the source are left untouched on the destination.
    """
