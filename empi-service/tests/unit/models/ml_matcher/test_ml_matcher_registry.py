...
def test_active_pointer_takes_precedence_over_mtime(tmp_path):
    a = _write_model(tmp_path, "ml_model_A.pkl", 0.78, 0.95)
    _write_model(tmp_path, "ml_model_B.pkl", 0.79, 0.95)
...
def test_meta_path_for_is_sibling_sidecar(tmp_path):
    assert R.meta_path_for(tmp_path / "ml_model_X.pkl").name == "ml_model_X.meta.json"


# ─── deploy gate ──────────────────────────────────────────────────────────────
def test_gate_passes_first_promotion(tmp_path):
    a = _write_model(tmp_path, "ml_model_A.pkl", 0.60, 0.60)
...
def test_gate_refuses_regression_beyond_margin(tmp_path):
    ...
    b = _write_model(tmp_path, "ml_model_B.pkl", 0.60, 0.95)  # precision drop 0.18 > margin
...
def test_gate_allows_within_margin(tmp_path):
    a = _write_model(tmp_path, "ml_model_A.pkl", 0.78, 0.95)
    R.promote(a, _settings(tmp_path))
    b = _write_model(tmp_path, "ml_model_B.pkl", 0.77, 0.94)  # within 0.02 margin
...
def test_force_promote_overrides_failed_gate(tmp_path):
    a = _write_model(tmp_path, "ml_model_A.pkl", 0.78, 0.95)
    R.promote(a, _settings(tmp_path))
    b = _write_model(tmp_path, "ml_model_B.pkl", 0.50, 0.50)
