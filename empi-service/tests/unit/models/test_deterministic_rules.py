...
Coverage:
    ...
    - Rule tiers: every rule today is auto-merge tier (no review-tier rule is
      currently defined — NAME_DOB_SEX / NAME_DOB_ADDRESS were removed).
...
class TestFuzzyNames:
    def test_single_typo_first_name_still_matches(self):
        # MEGAN vs MEAGAN (one inserted letter) → NAME_DOB_PHONE still fires.
        df = _clean_df([
...
class TestClassifyNonMatches:
    ...
    def test_reject_rule_threshold_matches_calibration(self):
        # The single formalized reject rule encodes the calibrated 3-contradiction
        # threshold over the four strong identifiers.
        (rule,) = REJECT_RULES
...
# ── Rule tiers ────────────────────────────────────────────────────────────────
class TestRuleTiers:
    """Every rule today is auto-merge tier. No review-tier rule is currently
    defined — NAME_DOB_SEX and NAME_DOB_ADDRESS, which used to fire but never
    auto-merge (~65% / ~67% silver precision), were removed; pairs they used
    to confirm now fall through to the downstream gate/ML matcher pipeline
    instead. These tests guard the (still-live) tier-partition machinery.
    """

    def test_tier_sets_partition_the_rules(self):
        assert AUTO_MERGE_RULES == {"SSN_DOB", "NAME_DOB_EMAIL", "NAME_DOB_PHONE"}
        assert REVIEW_RULES == set()
...
class TestSuspiciousFlag:
    ...
    def test_suspicious_when_both_ssn_present_differ(self):
        # Match via NAME_DOB_EMAIL, but both SSNs present and differing.
        a = {COL_PATID: "A", "FirstNM_clean": "JANE", "LastNM_clean": "DOE",
             "BirthDT_clean": pd.Timestamp("1990-01-01"),
             "Email_clean": "jane@x.com", "SSN_clean": "111111111"}
