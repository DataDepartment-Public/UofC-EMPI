"""Unit tests for the FS model registry: active-model resolution + deploy gate."""
...
from src.models.fs_matcher import registry as R


def _settings(model_dir: Path, active=None, margin=0.02):
    return SimpleNamespace(
        fs_active_model=active, fs_model_dir=model_dir, fs_deploy_gate_margin=margin,
    )
...
def test_resolve_falls_back_to_latest_by_mtime(tmp_path):
    _write_model(tmp_path, "fs_model_A.json", 0.78, 0.95)
    b = _write_model(tmp_path, "fs_model_B.json", 0.79, 0.95)
...
def test_active_pointer_takes_precedence_over_mtime(tmp_path):
    a = _write_model(tmp_path, "fs_model_A.json", 0.78, 0.95)
    _write_model(tmp_path, "fs_model_B.json", 0.79, 0.95)
...
def test_meta_path_for_is_sibling_sidecar(tmp_path):
    assert R.meta_path_for(tmp_path / "fs_model_X.json").name == "fs_model_X.meta.json"


# ─── deploy gate ──────────────────────────────────────────────────────────────
def test_gate_passes_first_promotion(tmp_path):
    a = _write_model(tmp_path, "fs_model_A.json", 0.60, 0.60)
...
def test_gate_refuses_regression_beyond_margin(tmp_path):
    ...
    b = _write_model(tmp_path, "fs_model_B.json", 0.60, 0.95)  # precision drop 0.18 > margin
...
def test_gate_allows_within_margin(tmp_path):
    a = _write_model(tmp_path, "fs_model_A.json", 0.78, 0.95)
    R.promote(a, _settings(tmp_path))
    b = _write_model(tmp_path, "fs_model_B.json", 0.77, 0.94)  # within 0.02 margin
...
def test_force_promote_overrides_failed_gate(tmp_path):
    a = _write_model(tmp_path, "fs_model_A.json", 0.78, 0.95)
    R.promote(a, _settings(tmp_path))
    b = _write_model(tmp_path, "fs_model_B.json", 0.50, 0.50)
