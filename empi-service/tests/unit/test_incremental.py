"""Unit tests for src/api/incremental.py — incremental single-record scoring
...
from src.api import incremental, publish, store
...
class TestAutoMerge:
    def test_new_record_joins_existing_entity_via_ssn_dob(self, conn, settings):
        ...
        existing_mid = store.get_entity_mid_for_patid(conn, "P1")
        ...
        assert store.get_entity_mid_for_patid(conn, "P6") == existing_mid
        entity = store.get_entity(conn, existing_mid)
    ...
    def test_bridges_two_previously_separate_entities(self, conn, settings):
        _seed_population(settings, conn)
        mid_12 = store.get_entity_mid_for_patid(conn, "P1")
        mid_3 = store.get_entity_mid_for_patid(conn, "P3")
        assert mid_12 != mid_3
        ...
        conn.execute(
            ...
            "VALUES ('B1', ?, 'P3')",
            (store.hash_block_key("234567891"),),
        )
        conn.commit()
        ...
        entity = store.get_entity(conn, target)
        ...
        assert store.get_entity(conn, absorbed) is None
...
class TestNoMatch:
    def test_unique_record_becomes_its_own_singleton(self, conn, settings):
        ...
        mid = outcomes[0]["mid"]
        entity = store.get_entity(conn, mid)
...
class TestReviewTier:
    def test_name_dob_sex_only_match_writes_review_candidate_not_auto_merge(
        ...
        candidates = store.review_candidates_for_patid(conn, "P9")
...
class TestStickyUnmerge:
    def test_locked_candidate_is_not_auto_merged_into(self, conn, settings):
        ...
        conn.execute(
            "INSERT OR IGNORE INTO block_key (block_id, key_value, patid) "
            "VALUES ('B1', ?, 'P3')",
            (store.hash_block_key("512345678"),),
        )
        conn.commit()
        ...
        assert store.get_entity_mid_for_patid(conn, "P10") != mid_3
        entity = store.get_entity(conn, mid_3)
