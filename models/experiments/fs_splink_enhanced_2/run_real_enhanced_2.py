...
# Records frame for split-training. Contains the deconstructed UNION of the
# train + test paired CSVs (~100k unique records), so labels' PATIDs from
# *either* split resolve. Generated by `scripts/build_synthetic_records.py`.
# (The older `synthetic_blocking_testing.csv` only contains the test split's
# records — wrong for m-training, see Phase E2-5-fix3 commit.)
DEFAULT_LABELS_RECORDS = (
    PROJECT_ROOT / "data" / "synthetic" / "synthetic_records_v3.csv"
)
