"""run_real_enhanced_4.py — score the real cohort with FSEnhanced4 (Phase E4-4).
...
enhanced_4 trains m supervised from the **real-cohort silver labels**
...
the cohort, with SSN/Email exact levels grounded to 1/n_distinct.
...
The corroboration gate (applied inside FSEnhanced4.classify) demotes
uncorroborated auto_merge pairs to human_review — test metrics and the
full-pool gate_demotions_full_pool count in diagnostics both reflect the gate.
...
Outputs:
    models/outputs/fs_splink_enhanced_4__<data-version>[_full_pool].parquet
        5-column cross-model eval schema (validation notebook reads this).
    data/matches_model_v2/fs_splink_enhanced_4_matches_model__<data-version>[_full_pool].parquet
        ProbabilisticMatches contract (validated).
    models/artifacts/fs_splink_enhanced_4/diagnostics__<data-version>[_full_pool].json
        Non-PHI: tier counts, score quantiles, trained m/u settings, grounded_u,
        gate_demotions_full_pool, untrained_u_after_training, and the
...
from models.experiments.fs_splink_enhanced_4.fs_enhanced_4 import (  # noqa: E402
    MODEL_NAME,
    FSEnhanced4,
)
...
# Production default (multi-core VM). FSEnhanced4's _estimate_u_with_guard
...
def _score_pairs_with_trained_model(
    ...
    NOTE: model._gate_df_clean is set by _train_and_score (which bypasses run())
    and persists throughout the runner's lifetime, so the corroboration gate
    fires correctly on the test-split pairs here too.
    """
...
def _evaluate_test_split(
    ...
    The corroboration gate fires inside model.classify() (via
    _score_pairs_with_trained_model), so the confusion matrix and metrics here
    reflect the gate. That is the intended behaviour: test metrics measure the
    full enhanced_4 system, not just the FS score.
    """
...
def _train_and_score(
    ...
    label_col: str,
    u_max_pairs: float,
) -> tuple[FSEnhanced4, pd.DataFrame, pd.DataFrame, pd.DataFrame, object]:
    """Train on the train split + score the production pool.

    Returns (model, classified, prob_matches, eval_schema, linker).

    IMPORTANT: model._df_clean and model._gate_df_clean are set here (before
    build_linker) because this path bypasses FSEnhanced4.run(), which would
    otherwise set them. Without arming:
      (a) build_settings() cannot ground SSN/Email exact-level u from the real
          cohort's n_distinct — it would fall back to hardcoded fallback values.
      (b) FSEnhanced4.classify() would skip the corroboration gate entirely
          (gate_frame is None), so both production scoring AND the held-out
          test-split metrics would not reflect the gate.
    Because model is returned and reused in _evaluate_test_split (via
    _score_pairs_with_trained_model → model.classify), _gate_df_clean must NOT be
    cleared here — the gate must also fire on test-split pairs.
    """
    model = FSEnhanced4(
    ...
    # Arm the model with the production cleaned frame BEFORE build_linker so that:
    # (a) build_settings() grounds SSN/Email exact u from real 1/n_distinct.
    # (b) classify() applies the corroboration gate against the production frame.
    # These calls bypass run(), which would otherwise set both attributes.
    model._df_clean = df_clean        # so build_settings grounds SSN/Email u from real 1/n_distinct
    model._gate_df_clean = df_clean   # authoritative frame for the corroboration gate in classify()
...
def main() -> None:
    ...
    model_for_prep = FSEnhanced4(labels_df=train_labels, label_col=args.label_col)
    df_model = model_for_prep.prepare_model_input(df_clean)

    logger.info(
        "Training FSEnhanced4 (u_max_pairs=%.0e, include_address=True)...",
        args.u_max_pairs,
    )
    try:
    ...
    # Gate demotion count on the full production pool (aggregate, non-PHI).
    if CORROBORATION_COL in classified.columns:
        gate_demotions_full_pool = int(
            (classified[CORROBORATION_COL] == DEMOTED_REASON).sum()
        )
    else:
        gate_demotions_full_pool = 0
    logger.info(
        "Corroboration gate demotions (full pool): %d", gate_demotions_full_pool
    )

    # Audit any non-null comparison levels left without a trained u.
    try:
        untrained_u = model.audit_untrained_u(linker)
    except Exception as exc:  # noqa: BLE001
        logger.warning("audit_untrained_u failed: %s — storing []", exc)
        untrained_u = []
    ...
    diagnostics = {
        ...
        # enhanced_4-specific diagnostics (aggregate, non-PHI).
        "grounded_u": model._grounded_u,
        "gate_demotions_full_pool": gate_demotions_full_pool,
        "untrained_u_after_training": untrained_u,
    }
    diagnostics["trained_settings"] = trained_settings
    with open(diag_path, "w") as f:
        # default=str keeps a non-JSON-serializable value inside trained_settings
        # from wiping the whole diagnostics file (test_metrics, grounded_u, gate
        # count) — it degrades that one value to its string form instead.
