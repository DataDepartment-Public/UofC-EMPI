"""tests/unit/test_fs_enhanced_4_real_runner.py

Unit tests for the PHI-free helpers in run_real_enhanced_4 (silver-label
...
from models.experiments.fs_splink_enhanced_4 import run_real_enhanced_4 as r
...
def test_binary_metrics_asymmetric():
    """fp != fn so precision != recall — catches a tp/fp/fn swap or a
    (tp+fp) <-> (tp+fn) denominator inversion that the symmetric cases miss."""
    y_true = pd.Series([1, 1, 0, 0, 0])
    y_pred = pd.Series([True, False, True, True, False])
    m = r._binary_metrics(y_true, y_pred)
    assert m == {
        "tp": 1, "fp": 2, "fn": 1, "tn": 1,
